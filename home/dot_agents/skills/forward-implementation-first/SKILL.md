...
- Keep parallel workers on light, nonoverlapping implementation and
  focused-test lanes.
- When the operator authorizes a worker pool of size N and useful bounded
...
- Forward progress must not wait for every parallel lane to finish. Consume
...
- If a heavy run exposes a defect in a stage, fix that stage before rerunning
  the cone.
