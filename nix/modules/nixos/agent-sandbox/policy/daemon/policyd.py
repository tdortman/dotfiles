...
class PolicyStore:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.ui_client_by_writer: dict[asyncio.StreamWriter, str] = {}
        self.ui_context_by_session: dict[str, UiSessionContext] = {}
        self.ui_spawn_last_by_uid: dict[int, float] = {}
        self.session_deny: dict[str, set[tuple[str, int]]] = {}
        self.session_sudo_allow: dict[str, set[tuple[str, ...]]] = {}
        self.session_sudo_deny: dict[str, set[tuple[str, ...]]] = {}
    ...
    def export_policy_files(
        ...
        if self.args.export_nix:
            ...
            for rule in net.get("allow", []):
                host = str(rule["host"]).replace('"', '\\"')
                port = int(rule["port"])
                lines.append(f'    {{ host = "{host}"; port = {port}; }}')
            lines.extend(["  ];", "  network.deny = ["])
            for rule in net.get("deny", []):
                host = str(rule["host"]).replace('"', '\\"')
                port = int(rule["port"])
                lines.append(f'    {{ host = "{host}"; port = {port}; }}')
            lines.extend(["  ];", "}", ""])
    ...
    def _session_denied(self, host: str, port: int) -> bool:
        active = set(self.ui_session_by_writer.values())
        if not active:
            return False
        keys = allow_keys(host, port)
        for session_id in active:
            bucket = self.session_deny.get(session_id)
            if bucket and any(key in bucket for key in keys):
                return True
        return False

    def has_omp_ui(self) -> bool:
        return any(kind == "omp" for kind in self.ui_client_by_writer.values())

    def _ui_notification_targets(self) -> list[asyncio.StreamWriter]:
        """OMP extension prompts only; kdialog UI when OMP is not connected."""
        writers = list(self.ui_clients)
        omp = [w for w in writers if self.ui_client_by_writer.get(w) == "omp"]
        if omp:
            return omp
        return [
            w for w in writers if self.ui_client_by_writer.get(w, "standalone") != "omp"
        ]

    def _disconnect_standalone_clients(self) -> None:
        for writer in list(self.ui_clients):
            if self.ui_client_by_writer.get(writer, "standalone") == "omp":
                continue
    ...
    def start_ui_session(
        ...
        *,
        ui_client: str = "standalone",
        cwd: str | None = None,
        home: str | None = None,
        project_root: str | None = None,
    ) -> str:
        if ui_client == "omp":
            self._disconnect_standalone_clients()
        session_id = uuid.uuid4().hex
        self.ui_clients.add(writer)
        self.ui_session_by_writer[writer] = session_id
        self.ui_client_by_writer[writer] = ui_client
    ...
    def end_ui_session(self, writer: asyncio.StreamWriter) -> None:
        ...
        self.ui_client_by_writer.pop(writer, None)
        session_id = self.ui_session_by_writer.pop(writer, None)
        if session_id:
            self.session_allow.pop(session_id, None)
            self.session_deny.pop(session_id, None)
            self.session_sudo_allow.pop(session_id, None)
            self.session_sudo_deny.pop(session_id, None)
    ...
    def _policy_denied(
        ...
        *,
        ...
        for rule in (merged.get("network") or {}).get("deny", []):
            if self._host_matches(str(rule["host"]), host) and int(rule["port"]) == port:
                return True
        return False

    def _sudo_policy_denied(
        self,
        argv: list[str],
        cwd: str | None,
        home: str | None,
        ...
        *,
        ...
        for rule in (merged.get("sudo") or {}).get("deny", []):
            if isinstance(rule, dict) and sudo_argv_matches(rule, argv):
                return True
        return False

    def _sudo_policy_allowed(
        self,
        argv: list[str],
        cwd: str | None,
        home: str | None,
        ...
        *,
        ...
        for rule in (merged.get("sudo") or {}).get("allow", []):
            if isinstance(rule, dict) and sudo_argv_matches(rule, argv):
                return True
        return False

    def _session_sudo_denied(self, argv: list[str]) -> bool:
        key = tuple(str(a) for a in argv)
        active = set(self.ui_session_by_writer.values())
        for session_id in active:
            bucket = self.session_sudo_deny.get(session_id)
            if bucket and key in bucket:
                return True
        return False

    def _session_sudo_allowed(self, argv: list[str]) -> bool:
        key = tuple(str(a) for a in argv)
        active = set(self.ui_session_by_writer.values())
        for session_id in active:
            bucket = self.session_sudo_allow.get(session_id)
            if bucket and key in bucket:
                return True
        return False
    ...
    def allow_source(
        ...
        *,
        ...
        if self._policy_denied(
            ...
            return "deny"
        if self._session_denied(host, port):
            return "deny"
        ...
        for rule in (merged.get("network") or {}).get("allow", []):
            if self._host_matches(str(rule["host"]), host) and int(rule["port"]) == port:
    ...
    def _maybe_spawn_ui(
        ...
        *,
        ...
        if not cmd or self.ui_clients or self.has_omp_ui():
            return
    ...
    async def notify_ui(self, payload: dict[str, Any]) -> None:
        targets = self._ui_notification_targets()
        if not targets:
            return
    ...
    async def request_network_approval(
        ...
        *,
        ...
        if not self.ui_clients:
            ...
            # register_ui already ran flush_pending_to_ui(); do not notify again.
        try:
    ...
    async def request_elevation(
        ...
        *,
        ...
        if self._sudo_policy_denied(
            argv, cwd, home, project_root, pid=pid, uid=uid
        ) or self._session_sudo_denied(argv):
            log(f"check deny sudo {' '.join(argv)} (policy)")
            return self._elevation_denied()
        if self._sudo_policy_allowed(
            argv, cwd, home, project_root, pid=pid, uid=uid
        ) or self._session_sudo_allowed(argv):
            return await self._exec_elevation(argv, cwd, home)
    ...
    def _persist_network_allow(
        ...
        *,
        ...
        net = current["network"]
        allow = {
            network_rule_key(r): r
            for r in net.get("allow", [])
            if isinstance(r, dict)
        }
        allow[(host.lower(), port)] = {
            "host": normalize_host(host),
            "port": port,
            "comment": label,
        }
        deny = {
            network_rule_key(r): r for r in net.get("deny", []) if isinstance(r, dict)
        }
        deny.pop((host.lower(), port), None)
        net["allow"] = sorted(allow.values(), key=lambda r: (r["host"], r["port"]))
        net["deny"] = sorted(deny.values(), key=lambda r: (r["host"], r["port"]))
        atomic_write_policy(path, current, home=home, owner_uid=owner_uid)

    def _persist_network_deny(
        self,
        path: Path,
        host: str,
        port: int,
        ...
        *,
        ...
        allow.pop((host.lower(), port), None)
        net["deny"] = sorted(deny.values(), key=lambda r: (r["host"], r["port"]))
        net["allow"] = sorted(allow.values(), key=lambda r: (r["host"], r["port"]))
        atomic_write_policy(path, current, home=home, owner_uid=owner_uid)

    def _persist_sudo_allow(
        self,
        path: Path,
        argv: list[str],
        label: str,
        *,
        ...
        allow = {
            k: r
            for r in sud.get("allow", [])
            if isinstance(r, dict) and (k := sudo_rule_key(r)) is not None
        }
        key = tuple(str(a) for a in argv)
        allow[key] = {"argv": list(argv), "comment": label}
        deny = {
            k: r
            for r in sud.get("deny", [])
            if isinstance(r, dict) and (k := sudo_rule_key(r)) is not None
        }
        deny.pop(key, None)
        sud["allow"] = sorted(allow.values(), key=lambda r: sudo_rule_key(r) or ())
        sud["deny"] = sorted(deny.values(), key=lambda r: sudo_rule_key(r) or ())
        atomic_write_policy(path, current, home=home, owner_uid=owner_uid)

    def _persist_sudo_deny(
        self,
        path: Path,
        argv: list[str],
        label: str,
        *,
        ...
        key = tuple(str(a) for a in argv)
        deny[key] = {"argv": list(argv), "comment": label}
        allow = {
            k: r
            for r in sud.get("allow", [])
            if isinstance(r, dict) and (k := sudo_rule_key(r)) is not None
    ...
    def _deny_network_scope(
        ...
        *,
        ...
        keys = allow_keys(host, port)
        if scope == "session":
            if not session_id or session_id not in set(
                self.ui_session_by_writer.values()
            ):
                return {
                    "ok": False,
                    "error": "session_id required (policy UI must be connected)",
                }
            bucket = self.session_deny.setdefault(session_id, set())
            for key in keys:
                bucket.add(key)
            allow_bucket = self.session_allow.get(session_id)
            if allow_bucket:
                for key in keys:
                    allow_bucket.discard(key)
        elif scope == "global":
            if not home:
                return {"ok": False, "error": "home required for global scope"}
            path = self._user_global_path(home)
            assert path is not None
            self._persist_network_deny(
                path, host, port, scope, home=home, owner_uid=owner_uid
            )
        elif scope == "project":
            if not project_root:
            ...
            return {"ok": False, "error": f"invalid scope: {scope}"}
        self.export_policy_files(cwd, home, project_root)
        self._audit("deny", host, port, scope)
        result: dict[str, Any] = {"ok": True, "host": host, "port": port, "scope": scope}
        if scope == "project" and project_root:
            try:
                result["path"] = str(
                    resolve_project_policy_path(project_root=project_root)
    ...
    def _approve_sudo_scope(
        ...
        *,
        ...
        if scope == "session":
            ...
            bucket = self.session_sudo_allow.setdefault(session_id, set())
            bucket.add(key)
            deny_bucket = self.session_sudo_deny.get(session_id)
            if deny_bucket:
                deny_bucket.discard(key)
        elif scope == "global":
            if not home:
                return {"ok": False, "error": "home required for global scope"}
            path = self._user_global_path(home)
            assert path is not None
            self._persist_sudo_allow(
                path, argv, scope, home=home, owner_uid=owner_uid
            )
        elif scope == "project":
            if not project_root:
                return {
            ...
            try:
                ...
                return {"ok": False, "error": str(exc)}
            self._persist_sudo_allow(
                project, argv, scope, home=home, owner_uid=owner_uid
            )
            log(f"project policy saved {project}")
        elif scope != "once":
            return {"ok": False, "error": f"invalid scope: {scope}"}
        self.export_policy_files(cwd, home, project_root)
        self._audit("approve", detail=json.dumps({"argv": argv, "scope": scope}))
        result: dict[str, Any] = {"ok": True, "argv": argv, "scope": scope}
        if scope == "project" and project_root:
            try:
                result["path"] = str(
                    resolve_project_policy_path(project_root=project_root)
    ...
    def _deny_sudo_scope(
        ...
        *,
        ...
        if scope == "session":
            ...
            bucket = self.session_sudo_deny.setdefault(session_id, set())
            bucket.add(key)
            allow_bucket = self.session_sudo_allow.get(session_id)
            if allow_bucket:
                allow_bucket.discard(key)
        elif scope == "global":
            if not home:
            ...
            return {"ok": False, "error": f"invalid scope: {scope}"}
        self.export_policy_files(cwd, home, project_root)
        self._audit("deny", detail=json.dumps({"argv": argv, "scope": scope}))
        result: dict[str, Any] = {"ok": True, "argv": argv, "scope": scope}
        if scope == "project" and project_root:
            try:
                result["path"] = str(
    ...
    async def approve(
        ...
        *,
        ...
        if scope != "once":
            scope_result = self._approve_sudo_scope(
                argv,
                scope,
                pending.cwd or cwd,
                pending.home or home,
            ...
            if not scope_result.get("ok"):
                ...
                return scope_result
            if scope_result.get("path"):
                saved_path = str(scope_result["path"])
        ...
        out: dict[str, Any] = {"ok": True, "allowed": True, "scope": scope}
        if saved_path:
            out["path"] = saved_path
        return out
        ...
        *,
        ...
        if pending.kind == "network":
            ...
            if scope == "once":
                ...
                return {"ok": True, "host": host, "port": port, "scope": scope}
            result = self._deny_network_scope(
                host,
                port,
                scope,
                pending.cwd or cwd,
        ...
        if scope == "once":
            ...
            return {"ok": True, "argv": argv, "scope": scope}
        result = self._deny_sudo_scope(
            argv,
            scope,
            pending.cwd or cwd,
            pending.home or home,
...
class PolicyServer:
    ...
    async def _dispatch(self, req: dict[str, Any], writer: asyncio.StreamWriter) -> dict[str, Any]:
        ...
        if op == "register_ui":
            ui_client = str(req.get("ui_client", "standalone"))
            if ui_client == "standalone" and self.store.has_omp_ui():
                return {
                    "ok": False,
